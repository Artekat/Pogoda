...
public class Interpolacja extends JPanel {
	...
	public Interpolacja(String[] data, int temperature[], JFrame jfrm) {
		System.out.println("Interpolacja konstruktor");
		this.jfrm = jfrm;
...
class PaintDemo {
	...
	PaintDemo(final String[] data, final int[] temperature) {
		...
		final JFrame jfrm = new JFrame("Interpolacja");

		jfrm.getContentPane().setLayout(null);

		pp = new Interpolacja(data, temperature, jfrm);
